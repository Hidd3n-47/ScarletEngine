...
private:
     ...
     */
    static void RecursivelyOutputElement(std::ofstream& file, const XmlElement* element, const uint8_t level = 0);
     ...
     */
    static uint32_t RecursivelyDeserializeDocument(const vector<std::string>& lines, XmlElement** previousElement, const uint32_t startingIndex = 0);
     ...
     */
    static uint32_t PeekNextLines(const vector<std::string>& lines, uint32_t startingIndex, std::string& value);
    ...
    enum class LineType : uint8_t
    {
        ...
        VERSION_LINE,           // Line is specifying version XML : <?xml version="1.0"?>
    };
