...
struct RenderGroup
{
    WeakHandle<Resource::Material> material;
...
struct hash<Scarlet::RenderGroup>
{
    size_t operator()(const Scarlet::RenderGroup& renderGroup) const noexcept
    {
        ...
        SCARLET_ASSERT(renderGroup.material.IsValid() && "Invalid texture trying to be hashed.");
        ...
        hashValue ^= std::hash<void*>{}(renderGroup.material.GetRawPtr()) + 0x9e3779b9 + (hashValue << 6) + (hashValue >> 2);
